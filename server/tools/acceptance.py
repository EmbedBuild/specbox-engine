...
# =====================================================================
# Module-level implementation (UC-405: reusable by regenerate_evidence)
#
# These functions were previously nested closures inside
# register_acceptance_tools. They are lifted to module level so that
# run_acceptance_check_impl can be imported and reused by other tools
# (e.g. evidence_regen.regenerate_evidence) without going through the
# FastMCP @mcp.tool wrapper. The registered tool delegates here, so
# the public behaviour of run_acceptance_check is unchanged.
# =====================================================================
...
def run_acceptance_check_impl(
    ...
    for uc_id in uc_ids_to_check:
        ...
        md_lines.append("---")
        md_lines.append("*Generated by SpecBox Engine `/acceptance-check`*")
...
def get_acceptance_report_impl(
    project_path: str,
    uc_id: str,
) -> dict:
    ...
    available: list[str] = []
    if check_dir.is_dir():
        available = [d.name for d in check_dir.iterdir() if d.is_dir() and (d / "report.json").exists()]
...
def get_e2e_gap_report_impl(
    ...
    # Extract all UCs
    all_ucs = list(dict.fromkeys(uc.upper() for uc in re.findall(r"(UC-\d+)", prd_text, re.IGNORECASE)))
    ...
    for uc_id in all_ucs:
        ...
        for ac in criteria:
            ...
            has_test_evidence = any(
                "test" in e.lower() or "acceptance" in e.lower() or "e2e" in e.lower() for e in ac_evidence
            )
                ...
                plan_items.append(
                    {
                        "uc_id": uc["uc_id"],
                        "status": uc["status"],
                        "missing_acs": [{"ac_id": c["ac_id"], "description": c["description"]} for c in missing_acs],
