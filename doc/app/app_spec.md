...
- **`native_project_id_contract` = owner/repo + display slug** (2026-06-03, US-NATIVE-PROVISION / UC-818) — el `project_id` canónico almacenado en `public.projects` es `owner/repo` (case-preserving, identidad); el slug URL-safe (`owner-repo`) es una proyección derivada solo para URLs/visual. Punto único de normalización: `server/coordination/project_id.py` (engine = fuente de verdad; el panel consume el mismo contrato). Cero migración de ids existentes. Ref. `doc/decisions/native_project_id_contract.md`.
- **`native_provision_authority` = el engine auto-provisiona al creador como `project_admin` (excepción de bootstrap)** (2026-06-03, US-NATIVE-PROVISION / UC-820,821) — en una migración a Native que nace de cero, `start_migration_session` crea `public.projects` + añade al caller como `project_admin` server-side, antes del gate de membresía (rompe el huevo-gallina). El §6 del panel (*"panel = único editor de project_members"*) se acota a excepción de bootstrap: el panel sigue siendo editor de OTROS miembros; un proyecto pre-existente del que el caller no es miembro NO se auto-une (FORBIDDEN). Valida dev_token, escribe solo en el tenant, audit, no relaja deny_anon/service_role. Ref. `doc/decisions/native_project_id_contract.md`.
